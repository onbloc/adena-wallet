...
interface MethodSignature { [methodName: string]: string }
...
export function parseGRC721FileContents(contents: string): GRC721Meta | null {
  ...
  const tokenUriCheck = checkInterfaceImplementation(contents, grc721Meta.variableName, { TokenUri: 'TokenURI(tid TokenID) (string, error)' });

  const metadataCheck = checkInterfaceImplementation(contents, grc721Meta.variableName, { TokenMetadata: 'TokenMetadata(tid TokenID) (Metadata, error)' });
