...
import { StorageModelDataV025 } from './storage-model-v025';

/**
 * v025 — replace the sapphire-1 testnet with pearl-1.
 *
 * pearl-1 is a separate chain rather than a rename of sapphire-1, so sapphire
 * state cannot be carried over: addresses, established sites, tokens and
 * sessions are all chain-scoped. Anything pinned to sapphire-1 is therefore
 * dropped, and a user sitting on sapphire-1 is moved to the new default testnet.
 */
const OLD_CHAIN_ID = 'sapphire-1';
const NEW_CHAIN_ID = 'pearl-1';

export class StorageMigration025 implements Migration<StorageModelDataV025> {
  public readonly version = 25;

  async up(
    current: StorageModel<StorageModelDataV024>,
  ): Promise<StorageModel<StorageModelDataV025>> {
    if (!this.validateModelV024(current.data)) {
      throw new Error('Storage Data does not match version V024');
    }
    const previous: StorageModelDataV024 = current.data;
  ...
  private validateModelV024(currentData: StorageModelDataV024): boolean {
  ...
  private migrateNetworks(networks: StorageModelDataV024['NETWORKS']): NetworksModelV022 {
  ...
  private removeOldChainEstablishSites(
    sites: StorageModelDataV024['ESTABLISH_SITES'],
  ): EstablishSitesModelV022 {
  ...
  private removeOldChainTokenMetainfos(
    metainfos: StorageModelDataV024['ACCOUNT_TOKEN_METAINFOS'],
  ): AccountTokenMetainfoModelV022 {
  ...
  private removeOldChainSessions(sessions: StorageModelDataV024['SESSIONS']): SessionsModelV022 {
  ...
  private migrateGrc721Collections(
    collections: StorageModelDataV024['ACCOUNT_GRC721_COLLECTIONS'],
  ): AccountGRC721CollectionsV022 {
  ...
  private migrateGrc721PinnedPackages(
    pinned: StorageModelDataV024['ACCOUNT_GRC721_PINNED_PACKAGES'],
  ): AccountGRC721PinnedPackagesV022 {
