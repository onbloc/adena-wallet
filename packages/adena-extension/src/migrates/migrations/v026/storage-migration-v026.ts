...
import { StorageModelDataV026 } from './storage-model-v026';

/**
 * v026 — retire the pearl-1 testnet and the gnoland1 beta mainnet; add the
 * gnoland-1 mainnet.
 *
 * Neither retired chain is a rename of gnoland-1, so their state cannot be
 * carried over: established sites, tokens, sessions and GRC721 data are all
 * chain-scoped and are dropped. A user sitting on gnoland1 is moved to the new
 * mainnet, and a user sitting on pearl-1 is moved to the staging testnet so
 * they stay on the same mainnet/testnet side they were on.
 */
const OLD_BETA_MAINNET_ID = 'gnoland1';
const OLD_TESTNET_ID = 'pearl-1';
const NEW_MAINNET_ID = 'gnoland-1';
const NEW_TESTNET_ID = 'staging';

const CHAIN_ID_REPLACEMENTS: Record<string, string> = {
  [OLD_BETA_MAINNET_ID]: NEW_MAINNET_ID,
  [OLD_TESTNET_ID]: NEW_TESTNET_ID,
};

export class StorageMigration026 implements Migration<StorageModelDataV026> {
  public readonly version = 26;

  async up(
    current: StorageModel<StorageModelDataV025>,
  ): Promise<StorageModel<StorageModelDataV026>> {
    if (!this.validateModelV025(current.data)) {
      throw new Error('Storage Data does not match version V025');
    }
    const previous: StorageModelDataV025 = current.data;
  ...
  private validateModelV025(currentData: StorageModelDataV025): boolean {
  ...
  private isOldChainId(id: string): boolean {
    return id in CHAIN_ID_REPLACEMENTS;
  }

  private migrateChainId(id: string): string {
    return CHAIN_ID_REPLACEMENTS[id] ?? id;
  }

  private migrateNetworks(networks: StorageModelDataV025['NETWORKS']): NetworksModelV022 {
  ...
  private removeOldChainEstablishSites(
    sites: StorageModelDataV025['ESTABLISH_SITES'],
  ): EstablishSitesModelV022 {
  ...
  private removeOldChainTokenMetainfos(
    metainfos: StorageModelDataV025['ACCOUNT_TOKEN_METAINFOS'],
  ): AccountTokenMetainfoModelV022 {
  ...
  private removeOldChainSessions(sessions: StorageModelDataV025['SESSIONS']): SessionsModelV022 {
  ...
  private migrateGrc721Collections(
    collections: StorageModelDataV025['ACCOUNT_GRC721_COLLECTIONS'],
  ): AccountGRC721CollectionsV022 {
  ...
  private migrateGrc721PinnedPackages(
    pinned: StorageModelDataV025['ACCOUNT_GRC721_PINNED_PACKAGES'],
  ): AccountGRC721PinnedPackagesV022 {
