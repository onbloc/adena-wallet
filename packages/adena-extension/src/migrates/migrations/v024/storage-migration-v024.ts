...
import { StorageModelDataV024 } from './storage-model-v024';

/**
 * v024 — replace the topaz-1 testnet with sapphire-1.
 *
 * sapphire-1 is a separate chain rather than a rename of topaz-1, so topaz
 * state cannot be carried over: addresses, established sites, tokens and
 * sessions are all chain-scoped. Anything pinned to topaz-1 is therefore
 * dropped, and a user sitting on topaz-1 is moved to the new default testnet.
 */
const OLD_CHAIN_ID = 'topaz-1';
const NEW_CHAIN_ID = 'sapphire-1';

export class StorageMigration024 implements Migration<StorageModelDataV024> {
  public readonly version = 24;

  async up(
    current: StorageModel<StorageModelDataV023>,
  ): Promise<StorageModel<StorageModelDataV024>> {
    if (!this.validateModelV023(current.data)) {
      throw new Error('Storage Data does not match version V023');
    }
    const previous: StorageModelDataV023 = current.data;
  ...
  private validateModelV023(currentData: StorageModelDataV023): boolean {
  ...
  private migrateNetworks(networks: StorageModelDataV023['NETWORKS']): NetworksModelV022 {
  ...
  private removeOldChainEstablishSites(
    sites: StorageModelDataV023['ESTABLISH_SITES'],
  ): EstablishSitesModelV022 {
  ...
  private removeOldChainTokenMetainfos(
    metainfos: StorageModelDataV023['ACCOUNT_TOKEN_METAINFOS'],
  ): AccountTokenMetainfoModelV022 {
  ...
  private removeOldChainSessions(sessions: StorageModelDataV023['SESSIONS']): SessionsModelV022 {
  ...
  private migrateGrc721Collections(
    collections: StorageModelDataV023['ACCOUNT_GRC721_COLLECTIONS'],
  ): AccountGRC721CollectionsV022 {
  ...
  private migrateGrc721PinnedPackages(
    pinned: StorageModelDataV023['ACCOUNT_GRC721_PINNED_PACKAGES'],
  ): AccountGRC721PinnedPackagesV022 {
