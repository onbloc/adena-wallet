import { StorageMigration021 } from './storage-migration-v021';
...
function makeInput(overrides: Partial<typeof BASE_DATA> = {}) {
  return { version: 20 as const, data: { ...BASE_DATA, ...overrides } };
}

describe('StorageMigration021', () => {
  it('version is 21', () => {
    expect(new StorageMigration021().version).toBe(21);
  });

  it('returns a v021 model with SESSIONS initialized as empty object', async () => {
    const result = await new StorageMigration021().up(makeInput());
    expect(result.version).toBe(21);
    expect(result.data.SESSIONS).toEqual({});
  });

  it('preserves all v020 fields without loss', async () => {
    const result = await new StorageMigration021().up(makeInput());
  ...
  it('throws when required v020 keys are missing', async () => {
    const { KDF_SALT, ...withoutKdfSalt } = BASE_DATA;
    const bad: any = { version: 20, data: withoutKdfSalt };
    await expect(new StorageMigration021().up(bad)).rejects.toThrow(
      'Storage Data does not match version V020',
    );
  });

  it('throws when SERIALIZED is not a string', async () => {
    const bad: any = { version: 20, data: { ...BASE_DATA, SERIALIZED: null } };
    await expect(new StorageMigration021().up(bad)).rejects.toThrow(
      'Storage Data does not match version V020',
    );
  });
  ...
  it('throws when NETWORKS is not an array', async () => {
    const bad: any = { version: 20, data: { ...BASE_DATA, NETWORKS: {} } };
    await expect(new StorageMigration021().up(bad)).rejects.toThrow(
      'Storage Data does not match version V020',
    );
  });
