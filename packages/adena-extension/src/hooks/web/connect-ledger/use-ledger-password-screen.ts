...
export type UseLedgerPasswordScreenReturn = {
  pwdState: {
    value: string;
...
export const useLedgerPasswordScreen = (): UseLedgerPasswordScreenReturn => {
  const { walletService, accountService } = useAdenaContext();

  const { navigate, params } = useAppNavigate<RoutePath.WebConnectLedgerPassword>();
  ...
  const nextButtonClick = async (): Promise<void> => {
    ...
    if (validationState === 'FINISH') {
      ...
      navigate(RoutePath.WebConnectLedgerAllSet);
      return;
    }
  };
