import { torusSignerConfig } from '@common/config'
...
const useGoogleLoginScreen = (): UseGoogleLoginReturn => {
  ...
  const initGoogleLogin = useCallback(() => {
    ...
    navigate(RoutePath.WebQuestionnaire, {
      state: { callbackPath: RoutePath.WebGoogleLogin },
    })
  ...
  const requestGoogleLogin = async (): Promise<void> => {
    try {
      ...
      const web3Auth = GoogleTorusSigner.create(torusSignerConfig)
      console.log('web3Auth', torusSignerConfig)
  ...
  const _addGoogleAccount = useCallback(
    async (privateKey: string) => {
      ...
      const web3AuthKeyring = await Web3AuthKeyring.fromPrivateKeyStr(
        privateKey,
      )
      const account = await SingleAccount.createBy(
        web3AuthKeyring,
        clone.nextAccountName,
      )
