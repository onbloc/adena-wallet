...
export const InfoTooltipTooltipBoxWrapper = styled.div<TooltipBoxWrapperProps>`
  position: ${({
    $position,
  }): string => ($position?.left !== undefined ? 'fixed' : 'absolute')};
  bottom: ${({
    $position,
  }): string =>
  ...
  top: ${({
    $position,
  }): string => ($position?.top !== undefined ? `${$position.top}px` : 'auto')};
  left: ${({
    $position,
  }): string =>
  ...
  transform: ${({
    $position,
  }): string => $position?.transform || 'translate(-50%, -100%)'};
  ...
  &::after {
    ...
    left: ${({
      $arrowPosition,
    }): string => $arrowPosition?.left || '50%'};
    transform: ${({
      $arrowPosition,
    }): string => $arrowPosition?.transform || 'translateX(-50%)'};
    width: 0;
    height: 0;
