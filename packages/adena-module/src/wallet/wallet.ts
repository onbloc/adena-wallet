...
export class AdenaWallet implements Wallet {
  ...
  /**
   * Create a multisig account from existing address
   * Use this when you already have a multisig address from MultisigService
   * @param name - Account name
   * @param address - Multisig address (bech32)
   * @param signers - Array of signer addresses (bech32)
   * @param threshold - Required signature count
   * @returns Created MultisigAccount
   */
  async addMultisigAccountFromAddress(
    name: string,
    address: string,
    publicKey: Uint8Array,
    signers: string[],
    ...
    if (threshold < 1 || threshold > signers.length) {
      throw new Error(
        `Invalid threshold: must be between 1 and ${signers.length}, got ${threshold}`,
      );
    }
    ...
    // Create keyring from address
    const keyring = await MultisigKeyring.fromAddress(address, publicKey, multisigConfig);

    // Create account
    const account = await MultisigAccount.createBy(
      keyring,
      name || this.nextMultisigAccountName,
      this.lastGlobalAccountIndex + 1,
    );
  ...
  /**
   * Create a multisig sign request
   ...
   */
  async createMultisigSignRequest(
    accountId: string,
    document: Document,
  ): Promise<{
    unsignedTx: Tx;
    ...
    if (!(keyring instanceof MultisigKeyring)) {
      throw new Error('Keyring is not a MultisigKeyring');
    }

    return keyring.createSignRequest(document);
  }
   ...
   * @param signatures - Array of individual signatures with metadata
  ...
  async combineMultisigSignatures(
    ...
    // Combine signatures using keyring
    return keyring.combineSignatures(unsignedTx, signatures);
  }
   ...
   * @param accountId - Multisig account ID
   * @param signedTx - Signed multisig transaction
