...
import { rawSecp256k1PubkeyToRawAddress } from '@cosmjs/amino/build/addresses';
import { makeCosmoshubPath } from '@cosmjs/amino/build/paths';
import { encodeSecp256k1Signature } from '@cosmjs/amino/build/signature';
import { sortedJsonStringify, StdSignDoc } from '@cosmjs/amino/build/signdoc';
...
import {
  ...
  KdfConfiguration,
  supportedAlgorithms,
} from '@cosmjs/amino/build/wallet';

//step ridge clown proud cigar practice range umbrella voice kangaroo arctic ensure
...
export class Secp256k1HdWallet implements OfflineAminoSigner {
  ...
  public static mnemonicGenerate(length: 12 | 15 | 18 | 21 | 24 = 12): string {
    const entropyLength = 4 * Math.floor((11 * length) / 33);
    const entropy = Random.getBytes(entropyLength);
    const mnemonic = Bip39.encode(entropy);
  ...
  public async getPrivkey(_address: string): Promise<Uint8Array> {
    const accounts = await this.getAccountsWithPrivkeys();
    const account = accounts.find(({ address }) => address === _address);
    if (account === undefined) {
      throw new Error(`Address ${_address} not found in wallet`);
    }
  ...
  public static checkValidMnemonic(seed: string): boolean {
    seed.split(/\s+/).forEach((x: any, y: any) => {
      if (EnglishMnemonic.wordlist.find((el) => el === x) === undefined) {
        return false;
      }
      return;
    });
